...
from project_service.app.match.utils.competence_similarity import (
    CompetenceSimilarityUtils,
)
...
class TestConvertImportance:
    def test_convert_importance(self, utils):
        assert utils.convert_importance(importance=2, importance_sum=10) == Decimal(
            "0.2"
        )
...
class TestDotCompetence:
    ...
    def test_dot_competence_low_level(self, utils):
        result = utils.dot_competence(
            importance=6, level=UserCompetenceLevelType.LOW, importance_sum=12
        )
...
class TestExecute:
    def test_perfect_match(self, utils):
        ...
        user_comps = [
            UserCompetence(
                user_id=1, competence_id=1, level=UserCompetenceLevelType.HIGH
            ),
            UserCompetence(
                user_id=1, competence_id=2, level=UserCompetenceLevelType.HIGH
            ),
        ]
    ...
    def test_partial_match(self, utils):
        ...
        user_comps = [
            ...
            UserCompetence(
                user_id=1, competence_id=2, level=UserCompetenceLevelType.LOW
            ),
        ]
        score = utils.execute(project_comps, user_comps)
