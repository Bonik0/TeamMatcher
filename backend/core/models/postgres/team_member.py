...
class TeamMemberDB(Base):
    ...
    team_id: Mapped[int] = mapped_column(
        Integer, cascade_foreign_key("teams.id"), nullable=False
    )
    user_id: Mapped[int] = mapped_column(
        Integer, cascade_foreign_key("users.id"), nullable=False
    )
    project_role_id: Mapped[int] = mapped_column(
        Integer, cascade_foreign_key("project_roles.id"), nullable=False
    )
    ...
    project_role = many_to_one_relationship(
        "ProjectRoleAssociationDB", back_populates="team_members"
    )
