...
class MatchUsersUseCase(IUseCase):
    ...
    async def execute(self, session: AsyncSession, project_id: int) -> None:
        ...
        users = await self.user_repository.get_with_priorities_and_competences(session, project_id)
        ...
        self.logger.info(
            "\n".join(
                [user.model_dump_json(indent=1) for user in users]
            )
        )
        
        await self.project_repository.update_status(session, project_id, ProjectStatus.FORMATED)
        ...
        await self.project_repository.update_status(session, project_id, ProjectStatus.COMPLETED)
