...
class CompetenceSimilarityUtils(ICompetenceSimilarityUtils):
    ...
    def convert_level(self, level: UserCompetenceLevelType) -> Decimal:
        return self.user_competence_level_values.get(level, Decimal()).quantize(
            self.quantize
        )

    def convert_importance(self, importance: int, importance_sum: int) -> Decimal:
        return (Decimal(importance) / Decimal(importance_sum)).quantize(self.quantize)

    def get_importance_sum(
        self, project_competences: list[ProjectRoleCompetence]
    ) -> int:
        return sum(
            (
    ...
    def dot_competence(
        self, importance: int, level: UserCompetenceLevelType, importance_sum: int
    ) -> Decimal:
        return self.convert_importance(importance, importance_sum) * self.convert_level(
            level
        )
    ...
    def execute(
        ...
        return sum(
            (
                ...
                if project_competence.competence_id in competence_id_to_level
                else Decimal()
                for project_competence in project_competences
            )
