...
from core.entities import UserProjectScore, UserWithFormsAndCompetences, ProjectRoleWithCompetences, ProjectRole
from app.match.utils.interfaces import ICompetenceSimilarityUtils, IMatchUtils, IUserProjectRoleSimilarityUtils
import math
...
class MatchUtils(IMatchUtils):
    ...
    def get_max_teams_number(
        ...
        return math.ceil(
            users_count / sum(
                project_role.quantity_per_team for project_role in project_roles
            )
        )
    ...
    def assign_roles(
        ...
        max_user_score = max(
            user_score.role_score
            for user_score in users_project_scores
        )
        total_vacancies = sum(
            project_role.quantity_per_team * num_teams 
            for project_role in project_roles
        )
        user_ids = {
            user_project_score.user_id 
            for user_project_score in users_project_scores
        }
        user_nodes = {
        ...
        role_nodes = {
            project_role.id: f"role_{project_role.id}" 
            for project_role in project_roles
        }   
    ...
    def balance_teams(
        ...
        num_teams = len(teams)
        team_sums = [
            sum(user.competence_match for user in team)
            for team in teams
        ]
        ...
        while improved and iter_count < max_iter:
            ...
            for k in range(num_teams):
                ...
                for role in project_roles:
                    ...
                    user_max = next(
                        (
                            user for user in teams[max_index] if user.project_role_id == role.id
                        )
                        , None
                    )
                    if user_k and user_max and user_k.competence_match < user_max.competence_match:
                        new_sum_max = team_sums[max_index] - user_max.competence_match + user_k.competence_match
                        new_sum_k = team_sums[k] - user_k.competence_match + user_max.competence_match
                        ...
                        if abs(new_diff) < current_diff:
                            ...
                            teams[max_index][index_max], teams[k][index_k] = teams[k][index_k], teams[max_index][index_max]
                ...
                for role in project_roles:
                    ...
                    user_min = next(
                        (
                            user for user in teams[min_index] if user.project_role_id == role.id
                        )
                        , None
                    )
                    if user_k and user_min and user_k.competence_match > user_min.competence_match:
                        new_sum_min = team_sums[min_index] - user_min.competence_match + user_k.competence_match
                        new_sum_k = team_sums[k] - user_k.competence_match + user_min.competence_match
                        ...
                        if abs(new_diff) < current_diff:
                            ...
                            index_k = teams[k].index(user_k)
                            teams[min_index][index_min], teams[k][index_k] = teams[k][index_k], teams[min_index][index_min]
            ...
            if not improved:
                for role in project_roles:
                    users_max = [user for user in teams[max_index] if user.project_role_id == role.id]
                    users_min = [user for user in teams[min_index] if user.project_role_id == role.id]
                    for user_max in users_max:
                        for user_min in users_min:
                            if user_max.competence_match > user_min.competence_match:
                                new_sum_max = team_sums[max_index] - user_max.competence_match + user_min.competence_match
                                new_sum_min = team_sums[min_index] - user_min.competence_match + user_max.competence_match
                                new_diff = new_sum_max - new_sum_min
                                if abs(new_diff) < current_diff:
                                    idx_max = teams[max_index].index(user_max)
                                    idx_min = teams[min_index].index(user_min)
                                    teams[max_index][idx_max], teams[min_index][idx_min] = teams[min_index][idx_min], teams[max_index][idx_max]
                                    team_sums[max_index] = new_sum_max
                                    team_sums[min_index] = new_sum_min
                                    improved = True
                                    break
