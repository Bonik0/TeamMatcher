...
import { getURLtoEmailVetify, goToHomePage, setTokens } from "../utils/auth";
...
const LoginPage = () => {
	...
	const handleSubmit = async (e) => {
		...
		if (response.status === 429) {
			handleErrorsChange(
				`Превышенно количесво попыток. Попробуйте через ${
		...
		if (response.status === 422) {
			const error_field = response_dict["detail"][0]["loc"][1];
			const error_message = response_dict["detail"][0]["msg"];
			handleErrorsChange(error_message, error_field);
			return;
		}
			...
			<div style={defaultStyles.card}>
				...
				<form onSubmit={handleSubmit} style={defaultStyles.form}>
					<input
						...
						style={{
							...defaultStyles.input,
							borderColor: errors.email === false ? "#1e90ff" : "red",
						}}
				...
				<div style={styles.links}>
					<Link
						to={getURLtoEmailVetify("/register", formData.email)}
						style={styles.link}
						onMouseEnter={(e) => {
					...
					<Link
						to={getURLtoEmailVetify("/forgot-password", formData.email)}
						style={styles.link}
						onMouseEnter={(e) => {
							e.currentTarget.style.textDecoration = "underline";
						}}
